...
class LaserScan:
  ...
  def open_scan_panda(self, filename, pose, aug):
    """ Open raw scan and fill in attributes
    """
    # reset just in case there was an open structure
    self.reset()
    ...
    # put in attribute
    points = point_cloud    # get xyz
    remissions = pcloud.to_numpy(dtype=np.float32)[:, 3] / 255  # get remission
    self.set_points(points, remissions)
...
class SemLaserScan(LaserScan):
  ...
  def open_label(self, filename, pc_size):
    """ Open raw scan and fill in attributes
    """
    # check filename is string
    if not isinstance(filename, str):
    ...
    with gzip.open(filename, 'rb') as f:
      ...
      labels = labels.to_numpy(dtype=np.uint32)[:pc_size]
      labels = np.array([label_map[label[0]] for label in labels]).reshape(-1)
      # print('LABELS SHAPE: ', labels.shape)

    #TODO fix set label and label projection
    # set it
