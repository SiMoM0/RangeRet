...
class KNN(nn.Module):
  def __init__(self, params, nclasses, dataset='kitti'):
    super().__init__()
    print("*"*80)
  ...
  def forward(self, proj_range, unproj_range, proj_argmax, px, py):
    ...
    if self.dataset == 'pandaset':
        knn_argmax_out = knn_argmax_onehot[:, :-1].argmax(dim=1) # pandaset with class 0 valid
    else:
        knn_argmax_out = knn_argmax_onehot[:, 1:-1].argmax(dim=1) + 1 # semantickitti with class 0 unlabeled
